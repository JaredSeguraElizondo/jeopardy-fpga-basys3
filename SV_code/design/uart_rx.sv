module uart_rx #(
    ...
    // ------------------------------------------------------------
    // Generador de reloj de oversampling x16
    // ------------------------------------------------------------
    ...
    // Estados:
    // IDLE  -> espera línea en reposo y detecta start bit
    // START -> verifica start bit y espera medio bit
    // DATA  -> recibe 8 bits, uno por baud
    // STOP  -> espera bit de parada y publica el dato
    // ------------------------------------------------------------
    always_ff @(posedge clk) begin
        if (reset) begin
            rx_state            <= IDLE;
    ...
    // rx_data_rdy se activa 1 ciclo cuando termina recepción
    // ------------------------------------------------------------
    always_ff @(posedge clk) begin
        if (reset) begin
            rx_data_rdy <= 1'b0;
